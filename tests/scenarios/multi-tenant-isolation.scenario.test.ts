...
describe('Scenario: Multi-tenant isolation', () => {
  it('hides org-A transactions from an org-B context', async () => {
    ...
    const txnA = await engine.repositories.transaction.createPaymentIntent(
      { amount: 100, gateway: 'fake', methodKind: 'card', idempotencyKey: `pay-${orgA}-c_a`, data: { customerId: 'c_a' } },
      { organizationId: orgA, actorId: 'u1' },
    );
    const txnB = await engine.repositories.transaction.createPaymentIntent(
      { amount: 200, gateway: 'fake', methodKind: 'card', idempotencyKey: `pay-${orgB}-c_b`, data: { customerId: 'c_b' } },
  ...
  it('getAll returns only the caller org rows', async () => {
    ...
    await engine.repositories.transaction.createPaymentIntent(
      { amount: 100, gateway: 'fake', methodKind: 'card', idempotencyKey: `pay-${orgA}-c_a1`, data: { customerId: 'c_a1' } },
      { organizationId: orgA },
    );
    await engine.repositories.transaction.createPaymentIntent(
      { amount: 300, gateway: 'fake', methodKind: 'card', idempotencyKey: `pay-${orgA}-c_a2`, data: { customerId: 'c_a2' } },
      { organizationId: orgA },
    );
    await engine.repositories.transaction.createPaymentIntent(
      { amount: 500, gateway: 'fake', methodKind: 'card', idempotencyKey: `pay-${orgB}-c_b1`, data: { customerId: 'c_b1' } },
      { organizationId: orgB },
    );
  ...
  it('verify under the wrong org cannot find the transaction', async () => {
    ...
    const txnA = await engine.repositories.transaction.createPaymentIntent(
      { amount: 100, gateway: 'fake', methodKind: 'card', idempotencyKey: `pay-${orgA}-c_cross`, data: { customerId: 'c_cross' } },
      { organizationId: orgA },
    );
