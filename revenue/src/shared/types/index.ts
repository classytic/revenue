...
import type { HoldInfo } from './transaction.interface.js';
...
export type TransactionFlowValue = 'inflow' | 'outflow';

/** @deprecated Use TransactionFlowValue instead */
export type TransactionTypeValue = TransactionFlowValue;

// ============ UNIFIED TRANSACTION INTERFACE ============
// Export unified transaction interface for app-level schema
export type {
  ITransaction,
...
// Re-export from shared-types via transaction interface
export type { HoldInfo, ReleaseRecord } from './transaction.interface.js';
...
export interface TransactionDocument {
  ...
  flow: TransactionFlowValue;  // Directional flow: 'inflow' or 'outflow'
  tags?: string[];
  ...
  amount: number;
  currency: string;
  fee?: number;                        // Processing/gateway fees
  tax?: number;                        // Tax amount
  net: number;                         // Net amount after fees and tax
  ...
  // ===== ESCROW/HOLD (both names for compatibility) =====
  hold?: HoldInfo;                     // Legacy field name
  escrow?: HoldInfo;                   // Canonical field name

  // ===== REFERENCES =====
  sourceId?: string | ObjectId;
  sourceModel?: string;
  relatedTransactionId?: ObjectId;     // For refunds, linking to original

  // ===== LEGACY FIELDS (deprecated) =====
  category?: string;                   // Deprecated: use 'type' instead
...
export interface SplitInfo {
  _id?: ObjectId; // Optional ID for embedded split documents
  type: SplitTypeValue;
  recipientId: string;
  recipientType: string;
  rate: number;
...
export interface ModelsRegistry {
  ...
  Settlement?: MongooseModel<any>; // Settlement model (optional)
}
